...
export function IdealWeightCalculator() {
  ...
  const calculateRange = (idealWeightKg: number): [number, number] => {
    ...
    return [minWeight * 2.20462, maxWeight * 2.20462] // Return in lbs for consistency
  }
  ...
  const toggleUnitSystem = () => {
    ...
    if (height) {
      if (unitSystem === "imperial") {
        const [feet, inches] = height.split(",").map(Number)
        setHeight(((feet * 12 + (inches || 0)) * 2.54).toFixed(1))
      } else {
    ...
    <>
      ...
      <div className="max-w-2xl mx-auto mb-6 px-4">
        <p className="text-lg text-gray-800 bg-white bg-opacity-80 rounded-md p-4 shadow-sm border border-gray-200">
          <strong>Use our AI-Powered Ideal Weight Calculator to quickly discover your personalized ideal weight range.</strong> Based on factors like your height, gender, age, and body frame, this tool provides an estimate of your healthy weight range, offers insights into healthy weight ranges for your body type, and helps you set realistic weight goals for better health outcomes.
        </p>
      </div>
